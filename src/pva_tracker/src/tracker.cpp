...
int main(int argc, char** argv) {
    ...
    ros::Subscriber pva_sub = nh.subscribe(uav_name + "/pva_setpoint", 1, pvaCallback);
    ...
    ros::Subscriber position_sub = nh.subscribe(uav_name + "/mavros/local_position/pose", 1, positionCallback);
    ros::Subscriber velocity_sub = nh.subscribe(uav_name + "/mavros/local_position/velocity_local", 1, velocityCallback);

    att_ctrl_pub = nh.advertise<mavros_msgs::AttitudeTarget>(uav_name + "/mavros/setpoint_raw/attitude", 1);
    odom_sp_enu_pub = nh.advertise<nav_msgs::Odometry>(uav_name + "/odom_sp_enu", 1);
